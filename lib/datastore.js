...
  mq_ch_[channelName]  zset  <[message1, timestamp1], [message2, timestamp2], ...>
用途：用于频道消费消息的情况
添加：后台push的时候入栈，并发送通知给sever，server中的每个连接需要记录当前正在消费的seqid，和最新的seqid，如果当前有最新id小于当前的值，则需要继续消费。
更新：定时检查消息队列（需要依赖当前连接的所有的channel），集合超过一定量（如 1w条）则删除多余的（按 seqid 倒序）；
...
const redisConfig = Config.getConfig("redisConfig") || {};
