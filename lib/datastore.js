...
const Datastore = {
    ...
    getServerIds: function() {
        ...
        return new Promise(function(resolve, fail) {
            RedisHelper.getClient(redisConfig).then(function(client) {
                ...
                const redisKey = KEY_SERVER_PREFIX + "*";
                client.keys(redisKey, function(err, data) {
                    if (err) {
                        fail(err);
                        return;
                    }
                    const serverIds = [];
                    data.forEach((id) => {
                        serverIds.push(id.replace(KEY_SERVER_PREFIX, ""));
                    });
                    resolve(serverIds);
                });
            });
     ...
     * @param {Object} serverId
     */
    getServerHost: function(serverId) {

        return new Promise(function(resolve, fail) {
            RedisHelper.getClient(redisConfig).then(function(client) {
                
                const redisKey = KEY_SERVER_PREFIX + serverId;
                client.hget(redisKey, "serverHost", function(err, data) {
                    if (err) {
                        fail(err);
                        return;
                    }
