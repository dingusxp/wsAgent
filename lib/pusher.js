...
// latency sampling number
const latencySamplingNumber = Config.getConfig("latencySamplingNumber") || 100;
...
const markMessageAcked = function(clientId, ackData) {
    ...
    const clientSocket = Context.getClientContext(clientId);
    if (!clientSocket) {
        logger.trace(`client has gone away! clientId: ${clientId}`);
        return;
    }
    if (!ackData.messageId) {
        logger.trace(`bad ack data. no messageId given! clientId: ${clientId}`);
        return;
    }
    ...
    // 延迟情况采样
    if (serverContext.latencySampling.length < latencySamplingNumber) {
