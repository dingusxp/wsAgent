...
const wrapMessage = function(message) {
    ...
    if (typeof message.data === "object" && message.data.loader && message.data.buffer) {
        // 兼容 json 格式化过的 buffer，即 {type: "Buffer", data: [...]} 形式
        if (message.data.buffer.constructor !== Buffer && message.data.buffer.data) {
            message.data.buffer = Buffer.from(message.data.buffer.data);
        }
